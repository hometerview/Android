...
import com.ftw.hometerview.applelogin.AppleInteraction
import com.ftw.hometerview.applelogin.AppleLoginConst
import java.util.*

class AppleLoginDialog(context: Context, val interaction: AppleInteraction? = null) : Dialog(context) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        webView.loadUrl(
            AppleLoginConst.APPLE_AUTHURL
                    + "?response_type=code&v=1.1.6&response_mode=form_post&client_id="
                    + AppleLoginConst.APPLE_CLIENT_ID + "&scope="
                    + AppleLoginConst.APPLE_SCOPE + "&state="
                    + UUID.randomUUID().toString() + "&redirect_uri="
                    + AppleLoginConst.APPLE_REDIRECT_URI
        )
